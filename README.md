...
DSU-Permissive 面向 arm64 GKI，支持 Android 12 至 Android 16 及以上对应的 `android12-5.10`、`android13-5.15`、`android14-6.1`、`android15-6.6` 与 `android16-6.12` DDK target。它在 PID 1 的 first-stage 确认当前系统确实是 DSU 后，只修改返回给该进程的顶层 vbmeta 读取视图，让后期 `libfs_avb` 将本次 DSU 启动识别为 `VerificationDisabled`；随后在 `selinux_setup` 阶段优先让原厂 init 读取到临时的：
...
支持矩阵按设备出厂 Android 版本限定，不支持非 GKI 内核：
...
- 每个 GKI target 必须使用各自构建的 KO，不能跨内核版本复用模块。
