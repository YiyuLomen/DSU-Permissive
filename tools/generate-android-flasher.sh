...
用法：
  ...
  # 只封装一个 target
  tools/generate-android-flasher.sh --target <DDK target> --output <脚本>
      [--module <dsu_permissive.ko>] [--module-dir <目录>]
      [--loader <dsuinit>] [--magiskboot <静态 arm64 magiskboot>]
      [--selinux <0|1>] [--avb <0|1>]
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --target|--output|--loader|--module|--module-dir|--magiskboot|--selinux|--avb)
...
if [[ "$target" == "auto" ]]; then
    [[ -z "$module" ]] || fail "--target auto 不能与单独的 --module 组合"
    bundle_mode=auto
    selected_targets=("${supported_targets[@]}")
else
    is_supported_target "$target" || fail "不支持 DDK target：$target"
    bundle_mode=single
...
for selected_target in "${selected_targets[@]}"; do
    if [[ "$bundle_mode" == "single" && -n "$module" ]]; then
        module_path=$module
    else
...
configurator_sha256=$(hash_file "$root_dir/tools/configure-metadata.sh")
...
sed \
    ...
    -e "s|@CONFIGURATOR_SHA256@|$configurator_sha256|g" \
...
append_payload configurator "$root_dir/tools/configure-metadata.sh" "$temp_output"
...
echo "设备端会在备份和修补前按 uname -r 选择 KO；显式传入 --flash 才会写分区"
