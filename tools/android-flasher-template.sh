...
configurator_sha256='@CONFIGURATOR_SHA256@'
...
用法：
  sh $0 [--flash] [--partition <块设备>] [--slot current|a|b|none]
       ...
       [--no-config] [--allow-dsu] [--allow-locked] [--keep-work]
  sh $0 --verify-bundle

默认只自动提取当前 boot/init_boot、生成备份和补丁镜像，不写分区。
只有显式传入 --flash 才会刷入，并在刷入后回读相同长度进行 SHA-256 校验。
...
metadata_is_mounted() {
    grep -q '[[:space:]]/metadata[[:space:]]' /proc/mounts 2>/dev/null
}
...
extract_asset configurator "$work_dir/assets/configure-metadata.sh" "$configurator_sha256"
chmod 0755 "$work_dir/assets/magiskboot" "$work_dir/assets/dsuinit" \
    "$work_dir/assets/patch-init-boot-android.sh" \
    "$work_dir/assets/configure-metadata.sh"
...
verified_state=$(get_property ro.boot.vbmeta.device_state)
flash_locked=$(get_property ro.boot.flash.locked)
...
if [ -e /metadata/gsi/dsu/booted ] && [ "$allow_dsu" -eq 0 ]; then
    fail "检测到当前正在运行 DSU；目标分区尚未写入，请从正常系统/recovery 执行，确认风险后可用 --allow-dsu"
fi
if { [ "$verified_state" = "locked" ] || [ "$flash_locked" = "1" ]; } &&
    [ "$allow_locked" -eq 0 ]; then
    fail "设备报告 bootloader 已锁定，目标分区尚未写入；若外部签名链确实允许，可显式使用 --allow-locked"
fi

if [ "$configure_metadata" -eq 1 ]; then
    if metadata_is_mounted; then
        sh "$work_dir/assets/configure-metadata.sh" \
            --selinux "$selinux_value" --avb "$avb_value" ||
            fail "metadata 配置写入失败，目标分区尚未写入"
    elif [ "$selinux_value" = "1" ] && [ "$avb_value" = "1" ]; then
        warn "/metadata 未挂载，未创建配置文件；模块将按不存在时的默认 1/1 生效"
    else
        fail "/metadata 未挂载，无法保存非默认配置；请先挂载或使用 --no-config"
    fi
fi
...
echo "现在可以重启设备；SELinux/AVB 配置将在下次 DSU 启动读取"
